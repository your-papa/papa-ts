...
export async function createOllamaProviderDefinition(options?: BuiltInProviderOptions): Promise<ProviderDefinition> {
    ...
    return {
        ...
        embeddingModels: createEmbeddingFactories(
            embeddingEntries,
            (descriptor) => createOllamaEmbeddingFactory(descriptor, options),
        ),
...
async function resolveOllamaEntries(
    options?: BuiltInProviderOptions,
): Promise<{ chatEntries: BuiltInProviderModelMap; embeddingEntries: BuiltInProviderModelMap }> {
    ...
    if (providedChatModels && providedEmbeddingModels) {
        return { chatEntries: providedChatModels, embeddingEntries: providedEmbeddingModels };
    }
...
async function fetchOllamaModels(
    options?: BuiltInProviderOptions,
): Promise<{ chatModels: BuiltInProviderModelMap; embeddingModels: BuiltInProviderModelMap }> {
    const fetchImpl = options?.fetchImpl ?? globalThis.fetch;
    if (!fetchImpl) {
        throw new Error('Provide options.fetchImpl when running outside environments with global fetch (e.g., Node <18).');
    }

    const baseUrl = sanitizeBaseUrl(options?.baseUrl ?? process.env.OLLAMA_BASE_URL ?? OLLAMA_DEFAULT_BASE_URL);
