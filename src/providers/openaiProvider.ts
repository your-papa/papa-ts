...
export async function createOpenAIProviderDefinition(options?: BuiltInProviderOptions): Promise<ProviderDefinition> {
    ...
    return {
        ...
        embeddingModels: createEmbeddingFactories(
            embeddingEntries,
            (descriptor) => createOpenAIEmbeddingFactory(descriptor, options),
        ),
...
async function resolveOpenAIModelEntries(
...
async function fetchOpenAIModels(
    ...
    if (!apiKey) {
        throw new Error('OpenAI model discovery requires an API key. Set OPENAI_API_KEY or pass options.apiKey.');
    }

    const fetchImpl = options?.fetchImpl ?? globalThis.fetch;
    if (!fetchImpl) {
        throw new Error('Global fetch implementation missing. Provide options.fetchImpl to createOpenAIProviderDefinition.');
    }

    const baseUrl = sanitizeBaseUrl(options?.baseUrl ?? process.env.OPENAI_BASE_URL ?? OPENAI_DEFAULT_BASE_URL);
